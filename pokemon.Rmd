...
## Importation des donnéees et traitements
...
# on regarde le nombre de pokemosn "doublons" (numero de pokedex)
...
pokemon2 <-select(pokemon,-c(german_name, japanese_name, species, catch_rate, base_friendship, base_experience, growth_rate, egg_type_number, egg_type_1, egg_type_2, egg_cycles))
...
knitr::kable(nb_pokemon_gen) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))

## Afficher le nombre de pokemons par status
# on cree un tableau avec le nombre de pokemons par generation 
nb_pokemon_status <- pokemon2 %>% count(status)  
# on affiche la table obtenue
knitr::kable(nb_pokemon_status) %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
...
# representation graphique (avec rAmcharts) des genres dans le top
amBarplot(x = "type_1", y = "n", data = filter(pokemon_type_1), labelRotation = -45, main="Représentation des types de pokemons",xlab="Type",ylab="Nombre de types")
...
# representation graphique (avec rAmcharts) des genres dans le top
amBarplot(x = "type_2", y = "n", data = filter(pokemon_type_2), labelRotation = -45, main="Représentation des types de pokemons",xlab="Type",ylab="Nombre de types")
# on peut voir que 492 pokemons n'ont pas second type

## Boxplot de la taille des pokemons
amBoxplot(pokemon2$height_m, data=pokemon2, ylim = c(0,12), xlab="", ylab="Taille en mètre", main="Boxplot de la taille des pokemon")

## Boxplot du poids des pokemons
amBoxplot(pokemon2$weight_kg, data=pokemon2, ylim = c(0,600), xlab="", ylab="Poids en kg", main="Boxplot du poids des pokemon")
```
